...
export class HybridSearchEngine {
    ...
    try {
      ...
      if (topResults.length > 0) {
        console.log(
          ...
          `"${topResults[0].fileName}" (score: ${topResults[0].score.toFixed(3)})`
        );
      }
  ...
  private mergeResults(
    ...
    // Create a map to merge results by fileName
    ...
    keywordResults.forEach((result) => {
      ...
      if (mergedMap.has(result.fileName)) {
